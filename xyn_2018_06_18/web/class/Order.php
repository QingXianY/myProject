...
class Order{
    ...
    //获取订单列表
    public function getOrderList($page,$rows,$order_code,$student_id,$start_date,$end_date){
        $offset = ($page-1)*$rows;
        $connect= db_config::getInstance();
        $conn=$connect->connect_bdb();
        $where = "where 1=1 ";
        ...
        $orderList_sql = "select * from v_order_stu_pro ".$where." order by order_time desc "." limit $offset,$rows";  //sql查询语句，根据订单编号查询
        $orderList_rs=$conn ->query($orderList_sql);
        $orderListCount_sql = "select count(*) from v_order_stu_pro ".$where;  //sql查询语句，根据角色姓名模糊查询
    ...
    public function addOrder($student_id,$product_id){
        $connect= db_config::getInstance();
        $get_time = new get_time();
        $present = $get_time->get_now_sec();
        $conn=$connect->connect_bdb();
        $insertOrder_sql = "insert into xd_order(student_id,product_id,order_time,order_status) VALUE('$student_id','$product_id','$present',0)";
        ...
        if($insertOrder_rs){
            ...
            //新增明细记录
            $Detail=new Detail();
            $Detail->addDetail($student_id,'购物消费',0,$product_price,$present);
            //修改积分
            $Student = new Student();
            $Student->modifyStudentIntegral($student_id,0,$product_price,$present);

            return $connect -> out_msg(1,'下单成功!');
        }else{
    ...
    public function cancelOrder($order_code){
        ...
        $get_time = new get_time();
        $present = $get_time->get_now_sec();
        $updateOrder_sql = "update xd_order set order_status = 2 where order_code = '$order_code' ";
        ...
        if($updateOrder_rs){
            ...
            $student_id = $this->selectOrderById($order_code)['student_id'];

            //新增明细记录
            $Detail=new Detail();
            $Detail->addDetail($student_id,'订单退款',1,$product_price,$present);

            //修改积分
            $Student = new Student();
            $Student->modifyStudentIntegral($student_id,1,$product_price,$present);
    ...
    public function checkOrder($order_code){
        $connect= db_config::getInstance();
        $conn=$connect->connect_bdb();
        $updateOrder_sql = "update xd_order set order_status = 1 where order_code = '$order_code' ";
        $conn ->query($updateOrder_sql);
        $updateOrder_rs = mysqli_affected_rows($conn);
        $conn -> close();
        if($updateOrder_rs){
    ...
    public function deleteOrder($order_code){
        $connect= db_config::getInstance();
        $conn=$connect->connect_bdb();
        $deleteOrder_sql = "delete from xd_order where order_code = '$order_code'";
