...
class Detail{
    ...
    //获取用户积分明细列表
    public function getDetailList($page,$rows,$student_id,$start_date,$end_date){
        $offset = ($page-1)*$rows;
        ...
        $detailList_sql = "select * from v_student_detail ".$where." order by detail_date desc"." limit $offset,$rows";  //sql查询语句，根据日期查询
        $detailList_rs=$conn ->query($detailList_sql);
        $detailList=array();
        $detailListCount_sql = "select count(*) from v_student_detail ".$where;  //sql查询语句，根据角色姓名模糊查询
    ...
    //新增明细
    public function addDetail($student_id,$detail_content,$detail_type,$detail_value,$detail_date){
        $connect= db_config::getInstance();
        $conn=$connect->connect_bdb();
        $insertDetail_sql = "insert into xd_detail(student_id,detail_content,detail_type,detail_value) VALUE('$student_id','$detail_content','$detail_type','$detail_value','$detail_date')";
    ...
    public function deleteDetail($rid){
        $connect= db_config::getInstance();
        $conn=$connect->connect_bdb();
        $deleteDetail_sql = "delete from xd_detail where rid = '$rid'";
