...
class Role{
    ...
    public function selectRoleByName($role_name){
        $selectRole_sql = "select * from xd_role where role_name = '$role_name'";
    ...
    public function selectRoleById($rid){
        $selectRole_sql = "select * from xd_role where rid = '$rid'";
        $connect= db_config::getInstance();
        $conn=$connect->connect_bdb();
        $selectRole_rs=$conn ->query($selectRole_sql);
        return $selectRole_row = $selectRole_rs -> fetch_assoc();
    ...
    public function getRoleList($page,$rows,$role_name){
        ...
        $roleList_sql = "select * from xd_role ".$where." limit $offset,$rows";  //sql查询语句，根据角色姓名模糊查询
        $roleList_rs=$conn ->query($roleList_sql);
        $roleList=array();
        $roleListCount_sql = "select count(*) from xd_role ".$where;  //sql查询语句，根据角色姓名模糊查询
    ...
    //新增角色,参数包含权限id数组
    public function addRole($role_name,$role_amount,$competence_arry){
        $connect= db_config::getInstance();
        $conn=$connect->connect_bdb();
        if($this->selectRoleByName($role_name)){
            return $connect -> out_msg(2,'角色已存在!');
        }else{
            $insertRole_sql = "insert into xd_role(role_name,role_amount) VALUE('$role_name','$role_amount')";
            $conn ->query($insertRole_sql);
            $insertRole_rs=mysqli_affected_rows($conn);
            if($insertRole_rs){
                $rid = $this->selectRoleByName($role_name)['rid'];
                // 关闭自动提交
                ...
                for ($i;$i<$num;$i++){
                    ...
                    $insertCompetence_sql = "insert into xd_role_competence(rid,cid) VALUE('$rid','$cid')";
    ...
    public function modifyRole($rid,$role_name,$role_amount,$competence_arry){
        $connect= db_config::getInstance();
        $conn=$connect->connect_bdb();
        if($this->selectRoleByName($role_name)&&$this->selectRoleById($rid)['role_name']!=$role_name){
            return $connect -> out_msg(2,'角色已存在!');
        }else{
            $updateRole_sql = "update xd_role set role_name = '$role_name',role_amount='$role_amount' where rid = '$rid' ";
    ...
    public function deleteRole($rid){
        ...
        if($deleteAllMember_rs){
            $deleteRole_sql = "delete from xd_role where rid = '$rid'";
